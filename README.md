...
To remove a mod, delete its pak. Nothing else has to be undone.

## Building it yourself

The pak in each folder can be rebuilt from the source next to it. The one thing not in
this repo is the game's own files: the patcher needs the original widget as input, and you
take that from your own copy of the game.

1. Pull the `.uasset` (and `.uexp`) named in the mod's README out of your ZeroSpace install
   with [FModel](https://fmodel.app) or anything else built on CUE4Parse. The game's paks
   are UE 4.27 and not encrypted, so there is no key to find.
...
which jumps back when it is done. That way nothing else in the widget shifts around, and
every part of the asset we did not mean to touch is left exactly as the game shipped it.
Each build is then checked by reading the finished pak back with a second parser and
comparing it against the original.

## Notes

- These only change what you see on your own screen. Nothing is sent to the server and
  nothing changes for anyone else in your match.
- Unofficial, and not connected to Starlance Studios. Use at your own risk. A game update
  can break a pak until it is rebuilt.
- The source here is MIT licensed. ZeroSpace and its assets belong to Starlance Studios.

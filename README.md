...
- ✅ = Needs you to be the host.  
  Block Auto Chase does nothing at all when you join someone else's match, because unit
  behavior is decided by whoever hosts.  
  Team Colors is milder: hosting sets each player's color once and it stays, while joining
  recolors units one by one every couple of seconds, so a unit that has just been built keeps
  its normal color until the next pass.
